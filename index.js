...
module.exports = {
  construct: function(self, options) {
    ...
    var timing = options.cumulative || options['per-request'];
    var suggestions = options['index-suggestions'];
    if (!(timing || suggestions)) {
      self.apos.utils.error('apostrophe-profiler: you must include at least one of per-request, cumulative and index-suggestions in APOS_PROFILER.\n');
      process.exit(1);
    }

    // improve apostrophe-cursor once for each feature enabled

    if (suggestions) {
      self.suggestions = {};
      self.apos.define('apostrophe-cursor', require('./lib/suggestions-cursor.js'));
    }

    if (timing) {
      self.apos.define('apostrophe-cursor', require('./lib/timing-cursor.js'));
