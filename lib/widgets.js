...
module.exports = function(self, options) {
  ...
  _.each(widgetManagers, function(manager, name) {
    ...
    function timeSince(req, start, category) {
      ...
      if (!self.debugWidgets[name]) {
        self.debugWidgets[name] = { invocations: 0, time: 0 };
      }
      if (!req.debugWidgets[name][category]) {
        req.debugWidgets[name][category] = 0;
      }
      if (!self.debugWidgets[name][category]) {
        self.debugWidgets[name][category] = 0;
      }
      req.debugWidgets[name].invocations++;
      req.debugWidgets[name].time += (end - start);
      req.debugWidgets[name][category] += (end - start);
      self.debugWidgets[name].invocations++;
      self.debugWidgets[name].time += (end - start);
      self.debugWidgets[name][category] += (end - start);
    }
  });
  self.outputWidgets = function(req) {
    if (!req) {
      // Output the overall figures instead
      req = self;
    }
    if (process.env['WIDGET_TIMES']) {
      if (req !== self) {
        ...
        console.log('****** OVERALL WIDGET TIMES');
      }
